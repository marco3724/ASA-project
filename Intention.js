...
export class Intention{
    ...
    async revisePickUp(plan){
        ...
        while ( !plan.stop ) {
            ...
            if(parcelsOnTheWay.length>0 && believes.parcels.filter(p =>p.carriedBy === believes.me.id)){ //if there are parcerls very near during my path i also want to pick them up, but only if im carrying less than the max carrying parcels
                plan.stop = true
